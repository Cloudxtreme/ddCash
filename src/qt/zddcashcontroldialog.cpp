...
#include "ui_zddcashcontroldialog.h"
...
std::list<std::string> ZddCashControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZddCashControlDialog::listMints;

ZddCashControlDialog::ZddCashControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZddCashControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZddCashControlDialog::~ZddCashControlDialog()
{
    delete ui;
}

void ZddCashControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZddCashControlDialog::updateList()
{
...
void ZddCashControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZddCashControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZddCash_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZddCashControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZddCashControlDialog::GetSelectedMints()
{
...
void ZddCashControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
